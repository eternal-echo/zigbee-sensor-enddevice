...
static void zclSampleSw_UartCB(uint8 port, uint8 event);
  ...
  // On/Off test
  static uint16 zclSampleSw_TestAddr;
...
#ifdef ZDO_COORDINATOR
  // read event
  if ( events & SAMPLEAPP_READ_EVT )
  {
    zclSampleSw_ReadTest();
...
/**
 * @fn      zclSampleSw_ReadTest
 *
 * @brief   Send "read" Command to device
 */
static void zclSampleSw_ReadTest(void)
{
    ...
    readCmd->numAttr = 1;
    readCmd->attrID[0] = ATTRID_ON_OFF_SWITCH_ACTIONS;
...
/**
 * @fn      zclSampleSw_WriteTest
 *
 * @brief   Send "write" Command to device
 */
static void zclSampleSw_WriteTest(void)
{
    afAddrType_t destAddr;
    ...
    writeCmd->attrList[0].attrData = (uint8 *)osal_mem_alloc(sizeof(uint8));
    if(writeCmd->attrList[0].attrData == NULL)
      return;
    
    writeCmd->numAttr = 1;
    writeCmd->attrList[0].attrID = ATTRID_ON_OFF_SWITCH_ACTIONS;
    writeCmd->attrList[0].dataType = ZCL_DATATYPE_ENUM8;
    *(writeCmd->attrList[0].attrData) = txID;
