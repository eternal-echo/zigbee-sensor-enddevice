...
static void zclSampleLight_LevelTest(void);
...
//GP_UPDATE  
  ...
  osal_start_timerEx(zclSampleLight_TaskID, 
                     SAMPLEAPP_LEVEL_TEST_EVT, 
                     SAMPLEAPP_LEVEL_TEST_PERIOD);
#else
...
#ifdef ZDO_COORDINATOR
  // Level test
  if ( events & SAMPLEAPP_LEVEL_TEST_EVT )
  {
    zclSampleLight_LevelTest();
    
    osal_start_timerEx(zclSampleLight_TaskID, 
                       SAMPLEAPP_LEVEL_TEST_EVT, 
                       SAMPLEAPP_LEVEL_TEST_PERIOD);
    
    return ( events ^ SAMPLEAPP_LEVEL_TEST_EVT );
  }
#else
  // Rejoin
...
static void zclSampleLight_AdjustLightLevel( void )
{
  ...
  hwLight_SetLevel(HW_LIGHT_GPIO_P14, 
                  (uint8)((uint16)zclSampleLight_LevelCurrentLevel*100/254));
...
static void zclSampleLight_LevelTest(void)
{
    ...
    zclGeneral_SendLevelControlMoveToLevelRequest(SAMPLELIGHT_ENDPOINT, 
                                                  &destAddr,
    ...
    HalLcdWriteStringValue("Set Level:", level, 10, 4);
    
    if(level < 50) level = 254;
    else level -= 50;
}
